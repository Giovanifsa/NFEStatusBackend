...
public interface NFEStatusSnapshotEAO extends JpaRepository<NFEStatusSnapshot, Long>, CustomNFEStatusSnapshotEAO {
	@Query(" SELECT nfe "
			...
			+ "AND ( "
			+ "		?3 = FALSE "
			+ "		OR nfe.idNFEStatusSnapshot = ( "
			+ " 		SELECT sub_nfe "
			+ "			FROM NFEStatusSnapshot sub_nfe "
			+ "			WHERE sub_nfe.captureMoment = ( "
			+ "				SELECT MAX(cm_sub_nfe.captureMoment) "	
			+ "				FROM NFEStatusSnapshot cm_sub_nfe "
			+ "				WHERE cm_sub_nfe.authorizer = sub_nfe.authorizer "
			+ "			) AND sub_nfe.authorizer = nfe.authorizer "
			+ "		) "
			+ ") "
